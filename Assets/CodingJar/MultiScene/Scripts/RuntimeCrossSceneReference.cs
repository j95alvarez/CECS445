...
namespace CodingJar.MultiScene
{
    ...
    public struct RuntimeCrossSceneReference
    {
		...
		public RuntimeCrossSceneReference( Object from, string fromField, UniqueObject to )
		{
		...
		public void SetToNull()
		{
			...
			if ( !fromObject )
				throw new ResolveException( string.Format( "Cross-Scene Ref: {0}. fromObject was already null in SetToNull", ToString() ) );

			ResolveInternal( fromObject, null, _sourceField, this );
		}
		...
		public void Resolve()
		{
			var fromObject = this.fromObject;
			if ( !fromObject )
				throw new ResolveException( string.Format( "Cross-Scene Ref: {0}. fromObject is null.", this ) );
			...
			ResolveInternal( fromObject, toObject, _sourceField, this );
		}
...
#else
			else if ( !field.FieldType.GetTypeInfo().IsAssignableFrom( toObject.GetType().GetTypeInfo() ) )
#endif
			{
